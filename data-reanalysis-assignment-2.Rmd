...
whole_thing <- function(df, n){ #this function will act on the specified dataframe (will be d), n times (will be 1000)
  ...
  species <- unique(df$Species) #This splits up the original dataframe by pulling each species by name "Species" only once
  for(k in 1:length(species)){ 
    ...
    singleSpdat <- df%>%filter(Species == species[k]) #this creates a mini data frame made up of the individual species being anaylyzed at a time that only contains the data on one species at a time
    speed[k] <- singleSpdat$Running_Speed[1] #this creates a running speed column in the new data frame for the species being analyzed based on the running speed designated for the first individual in the species (all individuals in each species have the same running speed)
    number[k] <- nrow(singleSpdat) #this creates a column in the new dataframe that is the number of rows per species, or the number of observances per species
    for(i in 1:n){
    ...
    q <- quantile(v, c(0.05, 0.95), na.rm=TRUE) #creates 95% confidence intervals for each variance
...
<img src="img/Gallis2004img.jpg" width="450px"/> 
#####I am not sure if this will successfully load the image in my Rmd.
